...
async function main(){
  ...
  try{
    console.log('Running: ALTER TABLE public.users ADD COLUMN IF NOT EXISTS last_seen timestamp DEFAULT now();');
    await sql`ALTER TABLE public.users ADD COLUMN IF NOT EXISTS last_seen timestamp DEFAULT now();`;
    console.log('Done: last_seen column ensured on public.users');
  }catch(err){
