...
async function main(){
  ...
  try{
    console.log('Running: ALTER TABLE public.users ADD COLUMN IF NOT EXISTS is_online boolean DEFAULT false;');
    await sql`ALTER TABLE public.users ADD COLUMN IF NOT EXISTS is_online boolean DEFAULT false;`;
    console.log('Done: is_online column ensured on public.users');
  }catch(err){
