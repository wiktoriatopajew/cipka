...
async function main(){
  ...
  try{
    console.log('Running: ALTER TABLE public.users ADD COLUMN IF NOT EXISTS password text;');
    await sql`ALTER TABLE public.users ADD COLUMN IF NOT EXISTS password text;`;
    console.log('Done: password column ensured on public.users');
  }catch(err){
