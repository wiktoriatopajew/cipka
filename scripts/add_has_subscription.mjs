...
async function main(){
  ...
  try{
    console.log('Running: ALTER TABLE public.users ADD COLUMN IF NOT EXISTS has_subscription boolean DEFAULT false;');
    await sql`ALTER TABLE public.users ADD COLUMN IF NOT EXISTS has_subscription boolean DEFAULT false;`;
    console.log('Done: has_subscription column ensured on public.users');
  }catch(err){
