...
async function main(){
  ...
  try{
    console.log('Running: ALTER TABLE public.users ADD COLUMN IF NOT EXISTS created_at timestamp DEFAULT now();');
    await sql`ALTER TABLE public.users ADD COLUMN IF NOT EXISTS created_at timestamp DEFAULT now();`;
    console.log('Done: created_at column ensured on public.users');
  }catch(err){
